...
6. Multiple matches? Ask user via speech. One match? Proceed"""
    ...
    /**
     * Chat-only mode: plain conversation with no JSON parsing, no device actions.
     * Used when user switches to Chat Mode in the text input panel.
     */
    suspend fun chatOnly(message: String): String? {
        val chatSystemPrompt = """You are Max, a helpful AI assistant. 
...
Be concise — keep responses under 3-4 sentences since they display as subtitles on a small overlay.
Do NOT output JSON. Just respond naturally like a friendly assistant."""

        // Add to conversation history for context continuity
        conversationHistory.add("user" to message)

        return try {
            val response = GroqApiClient.agentChat(
                context, chatSystemPrompt, conversationHistory, message
            )
            if (response != null) {
